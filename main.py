...
class Bar:
    ...
    def showBar(self,x,y):
        pygame.draw.rect(Game.gameDisplay,BLACK,[x,y,self.length,self.height])

    def showBall(self,x,y):
        pygame.draw.circle(Game.gameDisplay,DARKGREEN,(int(x),int(y)),self.radius)
...
class Game():
    ...
    def gameLoop(self):
        ...
        while not gameExit:
            ...
            self.gameDisplay.fill(VLIGHTGRN)
            self.clock.tick(60)
        pygame.quit()
        quit()
